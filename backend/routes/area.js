...
const areaWithOwner = (area) => ({
  ...area,
  owner_name: store.users.find((user) => user.user_id === area.owner_id)?.full_name || 'Chưa phân công',
});

router.get('/', (req,res)=>res.json(store.areas.map(areaWithOwner)));
