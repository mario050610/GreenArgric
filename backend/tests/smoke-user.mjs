...
if (!groundedAdminResponse.ok || groundedAdmin.provider !== 'system' || groundedAdmin.reply.includes('Phạm Phước Nguyên') || !groundedAdmin.reply.includes('chính mình') || groundedAdmin.reply.includes('Nguồn tham khảo') || groundedAdmin.reply.includes('http')) throw new Error(`Private role answer failed: ${groundedAdminResponse.status} ${JSON.stringify(groundedAdmin)}`);
